...
- Treat GitHub as the sole source of truth. A repository change is never complete while it exists only in a local working tree: before reporting completion, commit every in-scope change on a non-protected branch, push that branch, open or update the required pull request, complete and record the applicable publication review, wait for required checks, merge only through the protected workflow, and verify the resulting default-branch commit and Pages deployment. If any remote step cannot be completed, report the work as blocked or pending rather than finished.
- Use the `ofunk-nvidia` GitHub identity exclusively for every remote write, branch, pull-request, review, and merge action in this professional repository. The `ofunk` identity is not authorised for this repository and must not be used.
- For an owner-authored change initiated in Codex, Codex is the review and execution interface: inspect the complete diff against the publication checklist, record the evidence and result in the pull request, wait for required checks, merge through the authenticated `ofunk-nvidia` GitHub connector, and verify `main` and Pages. The owner's instruction in the Codex task authorises this workflow unless the owner explicitly requests review only. GitHub does not permit an author to approve their own pull request, so a separate self-approval is neither required nor valid.
- External contributions still require `ofunk-nvidia` CODEOWNER approval. Never bypass required pull requests, publication checks, external-contribution review, fork-workflow approval, or default-branch protections. Do not grant write access to simplify a contribution.
