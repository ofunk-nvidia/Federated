...
- Treat GitHub as the sole source of truth. A repository change is never complete while it exists only in a local working tree: before reporting completion, commit every in-scope change on a non-protected branch, push that branch, open or update the required pull request, wait for required checks and review, merge only through the protected workflow, and verify the resulting default-branch commit and Pages deployment. If any remote step cannot be completed, report the work as blocked or pending rather than finished.
